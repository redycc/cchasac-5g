...
md_path = root / "HARL_paper_notes_zhtw.md"
html_path = root / "_HARL_paper_notes_zhtw.html"
pdf_path = root / "HARL_paper_notes_zhtw.pdf"

src = md_path.read_text(encoding="utf-8")

# Protect math from markdown processing (which would otherwise eat `_` etc.)
block_store = []
inline_store = []
...
# $ ... $ inline math (avoid matching empty / cross-line)
src = re.sub(r"(?<!\$)\$(?!\s)([^\$\n]+?)(?<!\s)\$(?!\$)", _stash_inline, src)
