"""
H-HASAC v5: Interference-penalized Manager, no goal penalty.

Changes vs v4:
  - beta=0: remove goal penalty (hard clip already enforces ceiling; penalty was
    always negative since actual_p~0.5 << target_p~1.0, crushing Worker rewards)
  - Remove diversity bonus (lambda_div=0): led to degenerate [0.3,1,1] strategy
  - Manager reward = sum(raw_rewards over K) - lambda_intf * avg_interference
    Gives Manager organic incentive to reduce power on high-interference agents
  - p_min=0.3 retained, K=20
"""
import sys
import os
import numpy as np
...
class HHASACRunner(OffPolicyHARunner):
    ...
    def __init__(self, args, algo_args, env_args, manager_args=None):
        ...
        self.mgr_lambda_intf = m.get("lambda_intf", 20.0)  # interference penalty weight
        ...
        self._mgr_act = None
        self._k_rewards = []
        self._k_intfs = []   # per-step interference sum (from infos)
        self._mgr_step = 0
    ...
    def run(self):
        ...
        print("finish warmup, start H-HASAC v5 training")
        ...
        for step in range(1, steps + 1):
            ...
            # Track system interference for Manager reward shaping
            raw_intf = infos[0][0].get("interference", 0.0) if infos is not None and len(infos) > 0 else 0.0
            self._k_intfs.append(float(raw_intf))
            ...
            if self._mgr_step % self.K == 0 and self._mgr_obs_start is not None:
                ...
                # Manager reward: raw K-step reward minus interference penalty
                avg_intf = float(np.mean(self._k_intfs)) if self._k_intfs else 0.0
                mgr_rew = float(np.mean(self._k_rewards)) * self.K - self.mgr_lambda_intf * avg_intf
...
def build_runner(num_env_steps=300_000, K=20, seed=42,
                 exp_name="h_hasac_v5", env_overrides=None):
    ...
    manager_args = {
        ...
        "lr": 3e-4,
        "gamma": 0.95,
        "lambda_intf": 20.0,  # interference penalty weight for Manager reward
    }
