"""
cc-HASAC v3 — built on baseline.py's metrics(), fixes the v2 failure modes.

v2 FAILED because:
  1. New random channel snapshot every episode → encoder never sees the same KPM
     twice → cannot learn a stable z mapping.
  2. z←0 beat learned z (sum_rate 34.1 vs 31.0) → encoder learned harmful z.
  3. Workers DO use z (shuffle hurts: 20.3 vs 31.0) → architecture is wired
     correctly; the problem is purely the encoder cannot learn.

FOUR FIXES vs v2:
  Fix 1 (critical) SNAPSHOT POOL : pre-generate N_POOL=50 snapshots at init,
                                    sample from the pool on reset. Eval uses a
                                    SEPARATE held-out pool (seed 9999, N=20).
                                    → encoder now sees repeated channels, so z
                                      becomes learnable.
  Fix 2 SEPARATE ENCODER LR      : encoder optimised at 1e-4, actor at 3e-4,
                                    with SEPARATE optimizers (decoupled, not the
                                    joint enc+actor optimizer of v2).
  Fix 3 K_HOLD = 50              : z is refreshed every 50 steps (was 10) → z is
                                    more of a slow context variable.
  Fix 4 z-REGULARIZATION         : add 0.001 * E[z^2] KL-to-N(0,1) penalty to the
                                    actor/encoder objective → keeps z bounded.
...
POLYAK      = 0.005
ALPHA_INIT  = 0.01
Z_KL_COEF   = 0.001                   # Fix 4
BUFFER_SIZE = 100_000
BATCH_SIZE  = 256
WARMUP      = 2_000
TRAIN_EVERY = 10
K_HOLD      = 50                      # Fix 3 (was 10)
NUM_STEPS   = 300_000
LOG_EVERY   = 10_000
EP_LEN      = 200
N_POOL      = 50                      # Fix 1
...
class GlobalContextEncoder(nn.Module):
    """
    Transformer encoder over BS cells.
    Input:  [B, N_BS, KPM_DIM]
    Output: [B, Z_DIM]
    Permutation-equivariant per-cell (self-attention), then mean-pool → z.
    """
    def __init__(self, d_model=32, n_heads=2, n_layers=2):
        super().__init__()
        self.input_proj = nn.Linear(KPM_DIM, d_model)
    ...
    def forward(self, kpm):                    # [B, N_BS, KPM_DIM]
        h = self.input_proj(kpm)               # [B, N_BS, d_model]
        h = self.transformer(h)                # [B, N_BS, d_model] ← attention over cells
        z = self.proj(h.mean(dim=1))           # [B, Z_DIM] ← mean-pool for invariance
        return z
...
class CCHASACv5Runner:
    def __init__(self):
        ...
        # ── Fix 2: separate optimizers, separate LRs ───────────────────────────
    ...
    def update(self):
        ...
        # ── Actor + Encoder update (Fix 2: separate optimizers, Fix 4: z-KL) ───
        ...
        if not a_loss.isnan():
            ...
            self.actor_opt.step()
            self.enc_opt.step()                       # separate enc step
    ...
    def run(self):
        ...
        log_path = os.path.join(RESULTS_DIR, "cc_hasac_v5_log.txt")
        rew_path = os.path.join(RESULTS_DIR, "cc_hasac_v5_rewards.npy")
        ...
        print(f"cc-HASAC v5 (Transformer enc, R1-partial, freq_sel) | device={DEVICE} | KPM={KPM_DIM} z={Z_DIM} "
