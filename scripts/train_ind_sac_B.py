...
WORKER_OBS  = KPM_DIM + N_BS          # 3 + 3 = 6  (kpm + agent-id onehot)
SHARE_OBS   = N_BS * KPM_DIM          # 9  (all-kpm concat, no z)
HIDDEN      = 128
LR          = 3e-4
GAMMA       = 0.99
POLYAK      = 0.005
...
class IndSACRunner:
    ...
    def run(self):
        ...
        log_path = os.path.join(RESULTS_DIR, "ind_sac_B_log.txt")
        rew_path = os.path.join(RESULTS_DIR, "ind_sac_B_rewards.npy")

        kpm = self._reset_train()
        ep_rew, done_eps, log_lines = 0.0, [], []
        print(f"Independent SAC B (R2 KPM-only, freq_sel, no z) | device={DEVICE} | obs={WORKER_OBS} "
        ...
        np.save(os.path.join(RESULTS_DIR, "ind_sac_B_final.npy"),
