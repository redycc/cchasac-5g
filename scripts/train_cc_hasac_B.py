...
Z_DIM       = 8
WORKER_OBS  = KPM_DIM + N_BS + Z_DIM  # 3+3+8 = 14
SHARE_OBS   = N_BS * KPM_DIM + Z_DIM  # 9+8 = 17
HIDDEN      = 128
ACTOR_LR    = 3e-4                    # Fix 2
ENC_LR      = 1e-4                    # Fix 2
GAMMA       = 0.99
...
class CCHASACv3Runner:
    def __init__(self):
        ...
        self.buf      = CCBufferV3()
        self.env      = CCEnv(cfg=Cfg(freq_selective=True), seed=SEED)
        self.eval_env = CCEnv(cfg=Cfg(freq_selective=True), seed=SEED + 9999)
    ...
    def run(self):
        ...
        log_path = os.path.join(RESULTS_DIR, "cc_hasac_B_log.txt")
        rew_path = os.path.join(RESULTS_DIR, "cc_hasac_B_rewards.npy")
        ...
        print(f"cc-HASAC B (R2 KPM-only, freq_sel) | device={DEVICE} | KPM={KPM_DIM} z={Z_DIM} "
