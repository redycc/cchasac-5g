"""
H-HASAC v6: Fairness-incentivized Manager, no goal penalty.

Changes vs v5:
  - Replace interference penalty with Jain fairness bonus:
    Manager reward = sum(raw_rewards over K) + lambda_fair * avg_jain_fairness
  - Fairness ∈ [0,1] from env info; Manager learns to equalize per-UE throughput
    across BSes, which naturally drives meaningful power differentiation.
  - v5's interference penalty throttled one agent causing net throughput loss;
    fairness bonus aligns Manager with actual system goal.
  - beta=0, p_min=0.3, K=20 unchanged
"""
import sys
import os
import numpy as np
...
class HHASACRunner(OffPolicyHARunner):
    ...
    def __init__(self, args, algo_args, env_args, manager_args=None):
        ...
        m = manager_args or {}
        self.K = m.get("K", 20)
        self.beta = 0.0  # no goal penalty — hard clip already enforces ceiling
        self.mgr_batch = m.get("batch_size", 128)
        self.mgr_warmup = m.get("warmup_steps", 500)
        self.mgr_lambda_fair = m.get("lambda_fair", 5.0)  # Jain fairness bonus weight
        ...
        self._mgr_act = None
        self._k_rewards = []
        self._k_fairs = []   # per-step Jain fairness (from infos)
        self._mgr_step = 0
    ...
    def _inject_subgoal(self, share_obs_np):
        ...
        # Enforce p_min floor: prevent Manager from silencing any agent
        subgoals[:, 0] = np.clip(subgoals[:, 0], 0.3, 1.0)
    ...
    def run(self):
        ...
        print("finish warmup, start H-HASAC v6 training")
        ...
        for step in range(1, steps + 1):
            ...
            # No goal penalty: hard clip already enforces p_max ceiling
            aug_rewards = rewards

            self._k_rewards.append(float(np.mean(rewards)))
            # Track Jain fairness for Manager reward shaping
            raw_fair = infos[0][0].get("fairness", 0.0) if infos is not None and len(infos) > 0 else 0.0
            self._k_fairs.append(float(raw_fair))
            ...
            if self._mgr_step % self.K == 0 and self._mgr_obs_start is not None:
                ...
                # Manager reward: raw K-step reward + Jain fairness bonus
                avg_fair = float(np.mean(self._k_fairs)) if self._k_fairs else 0.0
                mgr_rew = float(np.mean(self._k_rewards)) * self.K + self.mgr_lambda_fair * avg_fair
...
def build_runner(num_env_steps=300_000, K=20, seed=42,
                 exp_name="h_hasac_v6", env_overrides=None):
    ...
    manager_args = {
        ...
        "lr": 3e-4,
        "gamma": 0.95,
        "lambda_fair": 5.0,   # Jain fairness bonus weight for Manager reward
    }
