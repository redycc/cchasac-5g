"""
H-HASAC v3: Diversity-incentivized Manager with calibrated penalties.

Changes vs v2:
  - beta: 2.0 -> 0.5  (v2 goal penalty crushed Worker rewards; actual_p~0.5 vs target_p~1.0 gave -0.5/step)
  - lambda_div: 5.0 -> 1.5  (v2 pushed Manager to degenerate 0-vs-1 strategy to maximize std)
  - K=20, diversity bonus retained
"""
import sys
import os
import numpy as np
...
class HHASACRunner(OffPolicyHARunner):
    ...
    def run(self):
        ...
        print("finish warmup, start H-HASAC v3 training")
...
def build_runner(num_env_steps=300_000, K=20, beta=0.5, seed=42,
                 exp_name="h_hasac_v3", env_overrides=None):
