"""
H-HASAC v4: p_min floor to prevent Manager from silencing agents.

Changes vs v3:
  - p_min=0.3: sub-goal p_max_frac clipped to [0.3, 1.0] in _inject_subgoal
    (v3 Manager degenerated to [0, 1, 1] strategy, silencing one agent entirely)
  - beta=0.5, lambda_div=1.5, K=20 unchanged
"""
import sys
import os
import numpy as np
...
class HHASACRunner(OffPolicyHARunner):
    ...
    def __init__(self, args, algo_args, env_args, manager_args=None):
        ...
        self.mgr_lambda_div = m.get("lambda_div", 1.5)  # diversity bonus weight
    ...
    def run(self):
        ...
        print("finish warmup, start H-HASAC v4 training")
...
def build_runner(num_env_steps=300_000, K=20, beta=0.5, seed=42,
                 exp_name="h_hasac_v4", env_overrides=None):
    ...
    manager_args = {
        ...
        "lr": 3e-4,
        "gamma": 0.95,
        "lambda_div": 1.5,   # diversity bonus weight
    }
