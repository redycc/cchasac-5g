...
class IndSACRunner:
    ...
    def run(self):
        ...
        log_path = os.path.join(RESULTS_DIR, "ind_sac_log.txt")
        rew_path = os.path.join(RESULTS_DIR, "ind_sac_rewards.npy")

        kpm = self._reset_train()
        ep_rew, done_eps, log_lines = 0.0, [], []
        print(f"Independent SAC (no z) | device={DEVICE} | obs={WORKER_OBS} "
        ...
        np.save(os.path.join(RESULTS_DIR, "ind_sac_final.npy"),
