"""
cc-HASAC v13 — v11 + lower encoder LR after unfreeze + extended 500k training.

Motivation: v11 (33.43 bps/Hz final) showed oscillatory eval during encoder
fine-tuning phase due to aggressive encoder LR (1e-4 matching actor LR).
Large encoder gradient updates cause z to drift rapidly, breaking actor-z
coupling and causing large performance swings.

Fix vs v11:
  ENC_LR_FINETUNE = 1e-5 (10× lower than frozen phase ENC_LR)
  Applied only AFTER encoder unfreezes at step ENC_FREEZE_STEPS.
  This allows z to adapt gradually without catastrophic drift.

Also extends training to 500k steps for more convergence time.

Everything else identical to v11:
  - BC pre-train 1500 steps, α=0.001, ENC_FREEZE_STEPS=100k
  - DeepSet encoder, parameter-shared actor, centralised twin Q-critic
  - Snapshot pool (N=50 train, N=20 eval, seed 9999)
  - K_HOLD=50, Z_KL_COEF=0.001, R1-partial obs (7-dim)
"""
import sys, os
...
HIDDEN      = 128
ACTOR_LR        = 3e-4
ENC_LR          = 1e-4   # used during frozen phase (not applied to encoder)
ENC_LR_FINETUNE = 1e-5   # 10× lower: slow encoder adaptation after unfreeze
GAMMA       = 0.99
POLYAK      = 0.005
ALPHA_INIT  = 0.001   # Lower init: less entropy pressure on BC init
Z_KL_COEF   = 0.001
BUFFER_SIZE = 100_000
BATCH_SIZE  = 256
WARMUP      = 0           # Fixed: no random warmup; BC policy fills buffer
TRAIN_EVERY = 10
K_HOLD      = 50
NUM_STEPS   = 500_000
LOG_EVERY   = 10_000
...
ENC_FREEZE_STEPS = 100_000  # actor stabilises on stable BC z before encoder fine-tuning
...
class CCHASACv13Runner:
    ...
    def run(self):
        ...
        log_path = os.path.join(RESULTS_DIR, "cc_hasac_v13_log.txt")
        rew_path = os.path.join(RESULTS_DIR, "cc_hasac_v13_rewards.npy")

        print(f"cc-HASAC v13 (BC pre-train + enc-freeze + low enc_lr finetune) | device={DEVICE} | "
        ...
        print(f"  cc-HASAC v13: {sr_z:.4f} bps/Hz  "
              f"(gap vs wmmse = {100*(bl['wmmse']-sr_z)/bl['wmmse']:+.1f}%)",
              flush=True)
        print(f"\n  Reference  : Ind-SAC A=28.1  cc-HASAC A=26.9  "
              f"Ind-SAC B=33.6  v11=33.43  WMMSE=85.0", flush=True)

        np.save(os.path.join(RESULTS_DIR, "cc_hasac_v13_ablation.npy"),
