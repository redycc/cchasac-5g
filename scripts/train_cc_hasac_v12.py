"""
cc-HASAC v12 (N_BS=5) — Behavioural-Cloning (BC) warm-start + encoder freeze.
...
Z_DIM       = 8
WORKER_OBS  = KPM_DIM + N_BS + Z_DIM  # 20
SHARE_OBS   = N_BS * KPM_DIM + Z_DIM  # 43
HIDDEN      = 128
ACTOR_LR    = 3e-4
ENC_LR      = 1e-4
GAMMA       = 0.99
...
ENC_FREEZE_STEPS = 100_000  # Extended: actor fully converges before encoder unfreezes  # RL steps to keep encoder frozen after BC
...
class CCHASACv12Runner:
    def __init__(self):
        ...
        self.buf      = CCBuffer()
        self.env      = CCEnv(cfg=Cfg(N_BS=N_BS, N_UE=N_UE, freq_selective=True), seed=SEED)
        self.eval_env = CCEnv(cfg=Cfg(N_BS=N_BS, N_UE=N_UE, freq_selective=True), seed=SEED + 9999)
    ...
    def run(self):
        ...
        log_path = os.path.join(RESULTS_DIR, "cc_hasac_v12_log.txt")
        rew_path = os.path.join(RESULTS_DIR, "cc_hasac_v12_rewards.npy")

        print(f"cc-HASAC v12 (N_BS=5) (BC pre-train + enc-freeze) | device={DEVICE} | "
        ...
        print(f"  cc-HASAC v12 (N_BS=5): {sr_z:.4f} bps/Hz  "
        ...
        np.save(os.path.join(RESULTS_DIR, "cc_hasac_v12_ablation.npy"),
